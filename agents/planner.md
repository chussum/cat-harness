...
If your dispatch prompt already carries an injected `[blast-radius HINT]` block (ralplan may splice one
in), it carries the same HINT-only trust level as a self-run `graph query` — still verify anything
load-bearing with Read/Grep.
</code_exploration>
